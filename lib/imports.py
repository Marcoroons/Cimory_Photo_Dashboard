...
from lib import db
from lib.flags import gps_distance_km, DEFAULT_DAILY_LIMIT, DEFAULT_GPS_THRESHOLD_KM
...
CANONICAL_FIELDS = [
    ...
    ("region", "recommended", "Region or area"),
    ...
    ("latitude", "optional", "Latitude for GPS flags"),
    ("longitude", "optional", "Longitude for GPS flags"),
...
def build_rows(df: pd.DataFrame, mapping: dict, project_id: str, config: dict,
               keep_extras: bool, existing_counts: dict, existing_urls: set) -> tuple[list, dict]:
    ...
    and idempotency handling against the database happen here too.
    """
    ...
    for i in range(len(df)):
        ...
        rec["submission_date"] = _parse_date(rec.get("submission_date"))
        rec["captured_at"] = _parse_ts(rec.get("captured_at"))
    ...
    for rec in records:
        ...
        dist, gps_far = gps_distance_km(
            ...
            mcm_ref.get(mcm) if mcm else None, gps_threshold,
        )
...
def run_import(project_id, mapping, df, raw_bytes, filename, config, keep_extras,
    ...
    rows, flag_summary = build_rows(
        df, mapping, project_id, config, keep_extras, existing_counts, existing_urls
    )
